...
const ITUpdatesMain = ({ currentUser, onLogout }) => {
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      const [statsRes, teamRes, projRes, tasksRes] = await Promise.all([
        ...
        itUpdatesApi.getTeamOverview(),
        itUpdatesApi.getProjects(),
        itUpdatesApi.getTasks(),
      ]);
  ...
  const fetchTasksWithFilters = useCallback(async (filters) => {
    try {
      const res = await itUpdatesApi.getTasks(filters);
  ...
  const teamActivity = useMemo(() => {
    const d = dashboardData;
    if (d?.teamActivity?.length) return d.teamActivity;
    return (teamOverview || []).map((m) => ({
      user_id: m.user_id,
      username: m.username ?? m.assignee,
      profile_image: m.profile_image,
      in_progress_count: m.in_progress_tasks ?? m.in_progress ?? 0,
      completed_today: 0,
      total_assigned: m.total_tasks ?? m.total ?? 0,
    }));
        ...
        <div className="it-updates-sidebar-footer">
          <div className="it-updates-sidebar-user">
            ...
            <div className="it-updates-sidebar-user-info">
              ...
              {user?.role && (
                <div className="it-updates-sidebar-userrole">{user.role}</div>
              )}
            </div>
            {onLogout && (
              <button
                    ...
                    {teamActivity.map((member) => (
                      ...
                      >
                        ...
                        <div className="it-updates-team-stats">
                          ...
                          <span>Completed today: {member.completed_today ?? 0}</span>
                        </div>
                      </div>
                    ))}
                    {!teamActivity.length && (
